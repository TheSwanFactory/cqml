...
values_{KEY} = list(filter(None, all_values))
control_{KEY} = widgets.{WIDGET}(
    value=values[0],
...
def callback_{KEY}(x): grid.restore(filter=[['{KEY}', '==', x]])
dropdown = widgets.interact(callback_{KEY}, x=control_{KEY})
grid
"""
